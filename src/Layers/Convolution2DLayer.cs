...
namespace Zcu.Convsharp.Layer
{
    ...
    /// </summary>
    [Serializable]
    public class Convolution2DLayer : AbstractLayer, ILearnable
    {
        ...
        public Convolution2DLayer(Dimension inputDimension = null, int filterSize = DEFAULT_FILTER_SIZE,
        ...
        public override double[][][][] ForwardPropagation(double[][][][] input, int startIndex, int endIndex, bool predict = false)
        {
            ...
            if (IsDimensionChanged(currImageCount, currDepth, currWidth, currHeight))
                activations = Utils.Init4dArr(inputDimension.imageCount,
                    filterCount, outputSize, outputSize);
                ...
                for (int convChannel = 0; convChannel < filtersChannels; convChannel++)
                {
                    for (int y = 0; y < currWidth; y++)
                    {
                        ...
                        for (int x = 0; x < currHeight; x++)
                        {
                            ...
                            {
                                for (int x_off = xOffsetMin; x_off < xOffsetMax; x_off++)
                                {
                                    imgY = y + y_off;
                                    imgX = x + x_off;
                                    subY = middleHeightOfFilter + y_off;
                                    subX = middleWidthOfFilter + x_off;
                                    for (int c_imgs = 0; c_imgs < filtersChannels; c_imgs++)
                                    {
                                        imageGradient[i][c_imgs][imgY][imgX] += filters[c_imgs][convChannel][subY][subX] * gradientValue;
                                        dWeights[c_imgs][convChannel][subY][subX] += (lastInput[i][c_imgs][imgY][imgX] * gradientValue) * normValue;
                                        // regularization
                                        if (regularizer != null)
                                        {
                                            // lambda/m * W
        ...
        public override AbstractLayer Compile(AbstractLayer previousLayer)
        {
            ...
            filters = weightInitializer.Initialize(new Dimension(filterCount,
                inputDimension.depth, filterSize, filterSize));
        ...
        private void TestHyperParametersAndSetOutputSize(int inputWidth, int inputHeight)
        {
            ...
            {
                string msg = "Expected square image in 3D (channels, width, height) in Convolution2D.";
                Utils.ThrowException(msg);
            }
